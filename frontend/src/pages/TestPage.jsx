...
// // Updated ActionButton to use storedSubId prop and re-render on storage
// const ActionButton = ({ exam, storedSubId }) => {
//   const navigate = useNavigate();
//   const attempted = Boolean(storedSubId);
...
//   const now       = new Date();
//   const tooEarly  = now < scheduled;
//   const isDisabled = !attempted && tooEarly;

//   const handleClick = e => {
//     e.stopPropagation();
//     if (attempted) {
//       navigate(`/view-answers/${storedSubId}`);
//     } else if (!tooEarly) {
...
//       `}
//       title={isDisabled ? `Available at ${scheduled.toLocaleString()}` : ''}
//     >
...
//   const now       = new Date();
//   const tooEarly  = now < scheduled;
//   // only disable if it's before the start AND the exam isn't already attempted
//   const isDisabled = !exam.attempted && tooEarly;

//   const handleClick = e => {
//     e.stopPropagation();
//     if (exam.attempted) {
//       // already done → go view answers
//       navigate(`/view-answers/${exam.submissionId}`);
//     } else if (!tooEarly) {
//       // on time → open secure exam
//       const examId = exam._id || exam.examId;
//       const url    = `${window.location.origin}/give-exam/${examId}`;
//       const features = [
//         `left=0`,
...
//   // 1) Keep track of whether we've got a submission ID (either from props or localStorage)
...
//   // 2) Listen for cross‐tab/storage events so we update immediately
//   useEffect(() => {
...
//   // 3) Build the scheduled DateTime so we can disable “Start” before it’s live
//   const scheduled = useMemo(() => {
//     const dt = new Date(exam.scheduleDate);
//     const [hh, mm] = (exam.scheduleTime || '00:00').split(':').map(Number);
//     dt.setHours(hh, mm, 0, 0);
...
//   const tooEarly = new Date() < scheduled;
//   const isDisabled = !attempted && tooEarly;

//   // 4) Click handler: either view answers or pop open the test window
//   const handleClick = e => {
//     e.stopPropagation();
//     if (attempted) {
...
//         `fullscreen=yes`,`toolbar=no`,`menubar=no`,
